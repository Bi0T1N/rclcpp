...
Executor::add_callback_groups_from_nodes_associated_to_executor()
{
  for (auto & weak_node : weak_nodes_) {
    ...
    if (node) {
      auto node_base_type = std::dynamic_pointer_cast<rclcpp::node_interfaces::NodeBase>(node);
      node_base_type->for_each_callback_group(
...
Executor::add_node(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_ptr, bool notify)
{
  ...
  std::lock_guard<std::mutex> guard{mutex_};
  auto node_base_type = std::dynamic_pointer_cast<rclcpp::node_interfaces::NodeBase>(node_ptr);
  node_base_type->for_each_callback_group(
